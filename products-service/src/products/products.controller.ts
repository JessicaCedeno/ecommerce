...
export class ProductsController {
  ...
  findOne(@Param('id', uuidPipe) id: string): Promise<Product> {
  ...
  @ApiOperation({
    summary:
      'Atomically reserve (decrement) stock — internal use by orders-service',
  })
  @ApiResponse({
    status: 200,
    description: 'Stock reserved, returns updated product',
  })
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: 'Release (increment) stock — rollback path for failed orders',
  })
  @ApiResponse({ status: 200 })
  releaseStock(
    @Param('id', uuidPipe) id: string,
